...
exports.sweepTurnTimeouts = exports.exportReplay = exports.onChoiceCreate = exports.createBattleWithTeams = exports.onReplacementCreate = void 0;
...
async function fetchMoveData(moveId) {
    var _a, _b, _c, _d;
    // Check cache first
    ...
    try {
        ...
        const moveData = {
            id: data.name,
            name: ((_b = (_a = data.names) === null || _a === void 0 ? void 0 : _a.find((n) => n.language.name === 'en')) === null || _b === void 0 ? void 0 : _b.name) || data.name,
            ...
            pp: data.pp,
            description: ((_d = (_c = data.flavor_text_entries) === null || _c === void 0 ? void 0 : _c.find((f) => f.language.name === 'en')) === null || _d === void 0 ? void 0 : _d.flavor_text) || ''
        };
...
function addSpecialMoveEffects(move) {
    ...
    // Recoil moves
    if (id.includes('head-smash'))
        move.recoilFrac = 0.5;
    else if (id.includes('double-edge') || id.includes('brave-bird'))
        move.recoilFrac = 0.33;
    else if (id.includes('take-down') || id.includes('volt-tackle'))
        move.recoilFrac = 0.25;
    // Drain moves
    if (id.includes('drain') || id.includes('leech'))
        move.drainFrac = 0.5;
    else if (id.includes('mega-drain'))
        move.drainFrac = 0.5;
    // Miss recoil moves
    if (id.includes('jump-kick') || id.includes('high-jump-kick'))
        move.missRecoilFrac = 0.5;
...
function rollMultiHitCount(ctx) {
...
async function hasAnyUsableMove(side, ctx) {
    var _a;
    const active = side.team[0];
    if (!((_a = active === null || active === void 0 ? void 0 : active.moves) === null || _a === void 0 ? void 0 : _a.length))
        return false;
    for (const m of active.moves) {
        if (await hasUsableMove(side, m.id, ctx))
            return true;
    }
    return false;
}
function coerceIllegalToStruggle(ctx, act) {
    if (act.kind !== "move")
        return act;
    // We'll check legality in the resolver, but if no moves are usable, force Struggle
    return act;
}
function hasHealthyBench(side) {
    const t = side.team;
    for (let i = 0; i < t.length; i++) {
        if (!t[i].fainted && t[i].stats.hp > 0)
            return true;
    }
    return false;
}
function isSafeguarded(ctx, uid) {
    var _a, _b;
    const sg = ((_b = (_a = ctx.pub.field) === null || _a === void 0 ? void 0 : _a.safeguard) === null || _b === void 0 ? void 0 : _b[uid]) || 0;
    return sg > 0;
}
// Use this for *status application* gates (knows attacker ability + house rule)
function statusBlockedBySafeguard(ctx, attacker, defenderUid) {
    var _a;
    if (!isSafeguarded(ctx, defenderUid))
        return false;
    const aAbility = (((_a = attacker.team[0]) === null || _a === void 0 ? void 0 : _a.ability) || "").toLowerCase();
    if (HOUSE_RULES.infiltratorBypassesSafeguard && aAbility === "infiltrator")
        return false; // house-rule
    return true;
}
// Clamp −6..+6
function clampStage(n) { return Math.max(-6, Math.min(6, n)); }
function ensureBoosts(ctx, uid) {
    const p = ctx.pub[uid];
    if (!p.active.boosts)
        p.active.boosts = { atk: 0, def: 0, spa: 0, spd: 0, spe: 0 };
    return p.active.boosts;
}
function raiseStat(ctx, uid, stat, stages) {
...
function isMisted(ctx, uid) {
    var _a, _b;
    return (((_b = (_a = ctx.pub.field) === null || _a === void 0 ? void 0 : _a.mist) === null || _b === void 0 ? void 0 : _b[uid]) || 0) > 0;
}
// Drop blockers on target
function blocksDrop(uidSide, stat) {
    var _a;
    const ab = (((_a = uidSide.team[0]) === null || _a === void 0 ? void 0 : _a.ability) || "").toLowerCase();
    if (["clear body", "white smoke", "full metal body"].includes(ab))
        return { blocked: true, reason: "It doesn't go any lower!" };
    if (stat === "atk" && ab === "hyper cutter")
        return { blocked: true, reason: "Attack won't go lower!" };
    return { blocked: false };
}
// Lower with Mist + blockers + Defiant/Competitive trigger
function lowerStatWithMist(ctx, targetUid, stat, stages, attacker) {
    ...
    if (bl.blocked) {
        ctx.logs.push(bl.reason || `The stat drop was prevented!`);
        return false;
    }
    ...
    if (attacker)
        triggerDefiantCompetitive(ctx, targetUid);
    return true;
}
function triggerDefiantCompetitive(ctx, targetUid) {
    var _a;
    const side = (targetUid === ctx.a.uid) ? ctx.a : ctx.b;
    const ab = (((_a = side.team[0]) === null || _a === void 0 ? void 0 : _a.ability) || "").toLowerCase();
    if (ab === "defiant") {
...
// Convenience
function getAbility(side) { var _a; return (((_a = side.team[0]) === null || _a === void 0 ? void 0 : _a.ability) || "").toLowerCase(); }
/** ------------------- Type chart (compact) ------------------- */
const TYPE_CHART = {
...
function typeEffect(attacking, defending) {
    var _a;
    let mult = 1;
    for (const t of defending) {
        const row = TYPE_CHART[attacking] || {};
        mult *= ((_a = row[t]) !== null && _a !== void 0 ? _a : 1);
    }
    return mult;
}
/** ---------- Helpers ---------- */
function maskPublicView(p) {
    return {
        species: p.species,
        level: p.level,
        types: p.types,
...
function validateTeam(team) {
    ...
    for (const mon of team) {
        if (typeof (mon === null || mon === void 0 ? void 0 : mon.species) !== "string" || !Array.isArray(mon === null || mon === void 0 ? void 0 : mon.moves)) {
...
function redactPrivate(priv) {
    ...
    for (const uid of Object.keys(priv)) {
        const p = priv[uid];
        out[uid] = {
            team: ((_a = p.team) !== null && _a !== void 0 ? _a : []).map((mon) => {
                var _a;
                return ({
                    species: mon.species,
                    moves: ((_a = mon.moves) !== null && _a !== void 0 ? _a : []).map(m => ({ id: m.id, pp: m.pp }))
                });
            }),
            choiceLock: (_b = p.choiceLock) !== null && _b !== void 0 ? _b : null,
            disable: (_c = p.disable) !== null && _c !== void 0 ? _c : null,
            encoreMoveId: (_d = p.encoreMoveId) !== null && _d !== void 0 ? _d : null
        };
    }
    return out;
}
function hashState(pub, privA, privB) {
...
/** ------------------- Builders ------------------- */
function buildSideState(uid, pub, priv) {
    var _a, _b, _c;
    const p = pub[uid];
    const act = p.active;
    const team = ((_a = priv === null || priv === void 0 ? void 0 : priv.team) !== null && _a !== void 0 ? _a : []);
    return {
        uid,
        active: { species: act.species, hp: act.hp.cur, maxHp: act.hp.max, status: (_b = act.status) !== null && _b !== void 0 ? _b : null },
        pubVolatiles: { ...((_c = act.volatiles) !== null && _c !== void 0 ? _c : {}) },
        publicVolatilesChanged: false,
        priv: priv !== null && priv !== void 0 ? priv : { team: [], choiceLock: {}, disable: null, encoreMoveId: null },
        team
    };
}
/** ------------------- Action building & ordering ------------------- */
function normalizeChoice(ch, user) {
    var _a, _b, _c, _d;
    if (ch.action === "switch")
        return { kind: "switch", user, toIndex: (_b = (_a = ch.payload) === null || _a === void 0 ? void 0 : _a.switchToIndex) !== null && _b !== void 0 ? _b : 1 };
    return { kind: "move", user, moveId: String(((_c = ch.payload) === null || _c === void 0 ? void 0 : _c.moveId) || ""), target: ((_d = ch.payload) === null || _d === void 0 ? void 0 : _d.target) === "p1" ? "p1" : "p2" };
}
function stageMultiplier(stage) {
    // Gen-like stage multipliers for simplicity: -6..+6
    if (stage >= 0)
        return (2 + stage) / 2; // 0:1.0, +1:1.5, etc.
    return 2 / (2 - stage); // -1:2/3 ≈ 0.667, -2:0.5, etc.
}
function effectiveSpeed(mon, status, pubVolatiles) {
    var _a;
    let sp = mon.stats.spe;
    if (status === "PAR")
        sp = Math.floor(sp * 0.5);
    // Sticky Web: we model as public speedStage -1 on entry if grounded
    const sStage = (_a = pubVolatiles === null || pubVolatiles === void 0 ? void 0 : pubVolatiles.speedStage) !== null && _a !== void 0 ? _a : 0;
    sp = Math.max(1, Math.floor(sp * stageMultiplier(sStage)));
    return sp;
}
async function orderActions(ctx, a, b) {
    var _a, _b;
    ...
    if (isSwitchA && b2.kind === "move" && b2.moveId === "pursuit") {
        interrupts.push({ kind: "pursuitInterrupt", user: "B" });
    }
    if (isSwitchB && a2.kind === "move" && a2.moveId === "pursuit") {
        interrupts.push({ kind: "pursuitInterrupt", user: "A" });
    }
    // 2) Separate switches and moves (moves will be sorted)
    const switches = acts.filter(x => x.kind === "switch");
    const moves = acts.filter(x => x.kind === "move");
    ...
    if (moves.length === 2) {
        ...
        if (pr1 !== pr2)
            moves.sort((x, y) => pr2 - pr1);
        else {
            const monA = ((_a = ctx.a.team[0]) !== null && _a !== void 0 ? _a : null);
            const monB = ((_b = ctx.b.team[0]) !== null && _b !== void 0 ? _b : null);
            ...
            if (spA !== spB)
                moves.sort((x, y) => (y.user === "A" ? spA : spB) - (x.user === "A" ? spA : spB));
            else {
                if (nextRand(ctx.rng).float < 0.5)
                    moves.reverse();
            }
        }
    }
...
async function getPriority(ctx, user, moveId) {
    var _a;
    const data = await getMoveData(moveId);
    return (_a = data === null || data === void 0 ? void 0 : data.priority) !== null && _a !== void 0 ? _a : 0;
}
/** ------------------- Hazard helpers ------------------- */
function isFlyingOrLevitating(mon) {
    // Minimal: treat Flying type as not grounded (no abilities/items modeled here)
    return (mon.types || []).includes("Flying");
}
function applyEntryHazards(ctx, side, opp) {
    var _a, _b, _c, _d, _e;
    const hz = ((_b = (_a = ctx.pub.field) === null || _a === void 0 ? void 0 : _a.hazards) === null || _b === void 0 ? void 0 : _b[side.uid]) || { sr: false, spikes: 0, tSpikes: 0, web: false };
    ...
    if (grounded) {
        ...
        if (hz.tSpikes && hz.tSpikes > 0 && side.active.hp > 0) {
            ...
            else if (!isSteel && !((_c = mon.types) === null || _c === void 0 ? void 0 : _c.includes("Flying")) && !side.active.status) {
                // We'll apply regular poison (badly poisoned requires tracking a counter)
                if (!statusBlockedBySafeguard(ctx, { team: [mon] }, side.uid)) {
        ...
        if (hz.web) {
            const cur = (_e = (_d = side.pubVolatiles) === null || _d === void 0 ? void 0 : _d.speedStage) !== null && _e !== void 0 ? _e : 0;
            if (cur > -1) {
...
/** ------------------- Resolution: switch ------------------- */
function performSwitch(ctx, user) {
    ...
    for (let i = 1; i < s.team.length; i++)
        if (!s.team[i].fainted && s.team[i].stats.hp > 0) {
            idx = i;
            break;
        }
    const tmp = s.team[0];
    ...
    if (s.active.hp <= 0)
        return; // fainted on entry; don't trigger abilities
...
async function resolvePursuitInterrupt(ctx, user) {
    ...
    if (!await hasUsableMove(atk, "pursuit", ctx))
        return;
    ...
    const boosted = { ...move, power: (move.power || 40) * 2 };
...
/** ------------------- Resolution: move ------------------- */
async function resolveMove(ctx, user, moveId, targetSideLabel) {
    var _a, _b, _c, _d, _e, _f;
    const atk = user === "A" ? ctx.a : ctx.b;
    const def = user === "A" ? ctx.b : ctx.a;
    const move = await getMoveData(moveId);
    if (!move) {
        ctx.logs.push(`${label(user, ctx)}'s move failed.`);
        return;
    }
    // Soundproof immunity to sound moves
    const defAbility = (((_a = def.team[0]) === null || _a === void 0 ? void 0 : _a.ability) || "").toLowerCase();
    ...
    // Protect logic:
    const defHasProtect = !!((_b = def.pubVolatiles) === null || _b === void 0 ? void 0 : _b.protectActive);
    ...
    if (move.category === "status") {
        ...
        const attackerAbility = (((_c = atk.team[0]) === null || _c === void 0 ? void 0 : _c.ability) || "").toLowerCase();
        const totalHits = typeof move.hits === "number" ? move.hits :
            move.hits === "2-5"
                ? (attackerAbility === "skill link" ? 5 : rollMultiHitCount(ctx))
                : 1;
        const atkAbility = (((_d = atk.team[0]) === null || _d === void 0 ? void 0 : _d.ability) || "").toLowerCase();
        ...
        for (let i = 1; i <= totalHits; i++) {
            ...
            const subRemaining = (_f = (_e = def.pubVolatiles) === null || _e === void 0 ? void 0 : _e.subHp) !== null && _f !== void 0 ? _f : 0;
            if (subRemaining > 0 && !bypassSub) {
                ...
                if (newSub <= 0)
                    ctx.logs.push(`${sideLabel(def, ctx)}'s Substitute faded!`);
            }
            else {
        ...
        if (totalDamage > 0)
            applyOnHit(ctx, atk, def, move, totalDamage, !anyHitThroughSub);
...
/** ------------------- Legality & PP ------------------- */
async function hasUsableMove(side, moveId, ctx) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j;
    // Recharge check (public volatile)
    if ((_a = side.pubVolatiles) === null || _a === void 0 ? void 0 : _a.recharge) {
        // Clear recharge for next turn at end-of-turn; for now we block usage
        return false;
    }
    const active = side.team[0];
    const move = (_b = active === null || active === void 0 ? void 0 : active.moves) === null || _b === void 0 ? void 0 : _b.find(m => m.id === moveId);
    if (!move || move.pp <= 0)
        return false;
    // Choice-lock (private)
    if (((_d = (_c = side.priv) === null || _c === void 0 ? void 0 : _c.choiceLock) === null || _d === void 0 ? void 0 : _d.locked) && ((_f = (_e = side.priv) === null || _e === void 0 ? void 0 : _e.choiceLock) === null || _f === void 0 ? void 0 : _f.moveId) && side.priv.choiceLock.moveId !== moveId) {
        return false;
    }
    // Disable (private)
    if (((_h = (_g = side.priv) === null || _g === void 0 ? void 0 : _g.disable) === null || _h === void 0 ? void 0 : _h.moveId) && side.priv.disable.moveId === moveId)
        return false;
    // Taunt (public) blocks typical status moves (heuristic; prefer tagging DB)
    const moveData = await getMoveData(moveId);
    if (((_j = side.pubVolatiles) === null || _j === void 0 ? void 0 : _j.taunt) && (moveData === null || moveData === void 0 ? void 0 : moveData.category) === "status")
        return false;
    return true;
}
function decrementPP(side, moveId) {
    const m = side.team[0].moves.find(mm => mm.id === moveId);
    if (m)
        m.pp = Math.max(0, (m.pp | 0) - 1);
    side.priv.team = side.team; // persist back
}
function onMissRecoil(side, move) {
...
function computeDamage(ctx, atk, def, move, crit, rand) {
    ...
    const aBoosts = ((_b = (_a = ctx.pub[atk.uid]) === null || _a === void 0 ? void 0 : _a.active) === null || _b === void 0 ? void 0 : _b.boosts) || { atk: 0, def: 0, spa: 0, spd: 0, spe: 0 };
    const dBoosts = ((_d = (_c = ctx.pub[def.uid]) === null || _c === void 0 ? void 0 : _c.active) === null || _d === void 0 ? void 0 : _d.boosts) || { atk: 0, def: 0, spa: 0, spd: 0, spe: 0 };
    ...
    if (ability === "technician" && techEligible)
        mod *= 1.5;
    // STAB
    if ((_e = aMon.types) === null || _e === void 0 ? void 0 : _e.includes(move.type))
        mod *= 1.5;
    // Type effectiveness (+ Tinted Lens)
    let typeMul = move.id === "struggle" ? 1 : typeEffect(move.type, (_f = dMon.types) !== null && _f !== void 0 ? _f : ["Normal"]);
    if (ability === "tinted lens" && typeMul > 0 && typeMul < 1)
        typeMul *= 2;
    mod *= typeMul;
    // Crit
    if (crit)
    ...
    if (!crit && move.category !== "status") {
        const atkAbility = (((_g = atk.team[0]) === null || _g === void 0 ? void 0 : _g.ability) || "").toLowerCase();
        const infiltrates = atkAbility === "infiltrator";
        if (!infiltrates) {
            const scr = ((_j = (_h = ctx.pub.field) === null || _h === void 0 ? void 0 : _h.screens) === null || _j === void 0 ? void 0 : _j[def.uid]) || { reflect: 0, lightScreen: 0 };
            if (move.category === "physical" && (scr.reflect | 0) > 0)
                mod *= 0.5;
            if (move.category === "special" && (scr.lightScreen | 0) > 0)
                mod *= 0.5;
        }
    }
    // Life Orb 1.3×
    if (item === "life orb" && move.category !== "status")
        mod *= 1.3;
...
function applyOnHit(ctx, atk, def, move, dmgTotal, hitSubstitute) {
    var _a, _b, _c;
    ...
    const item = (((_a = atk.team[0]) === null || _a === void 0 ? void 0 : _a.item) || "").toLowerCase();
    ...
    if (move.makesContact) {
        const defAbility = (((_b = def.team[0]) === null || _b === void 0 ? void 0 : _b.ability) || "").toLowerCase();
        const defItem = (((_c = def.team[0]) === null || _c === void 0 ? void 0 : _c.item) || "").toLowerCase();
        let hits = 0;
        if (defAbility === "rough skin")
            hits++;
        if (defItem === "rocky helmet")
            hits++;
        if (hits > 0) {
...
function applyStatusMove(ctx, atk, def, move) {
    var _a, _b, _c, _d, _e, _f;
    // Generic block: if defender has a Substitute, most status moves fail.
    // (You can whitelist exceptions later.)
    if (((_b = (_a = def.pubVolatiles) === null || _a === void 0 ? void 0 : _a.subHp) !== null && _b !== void 0 ? _b : 0) > 0) {
    ...
    switch (move.id) {
        ...
        case "will-o-wisp":
            ...
            if (!def.active.status) {
                def.active.status = "BRN";
                ctx.logs.push(`${sideLabel(def, ctx)} was burned!`);
            }
            break;
        case "thunder-wave":
            if (statusBlockedBySafeguard(ctx, atk, def.uid)) {
            ...
            if (!def.active.status) {
                def.active.status = "PAR";
                ctx.logs.push(`${sideLabel(def, ctx)} was paralyzed!`);
            }
            break;
        case "swords-dance":
            ...
            break;
        case "protect": {
            const chain = (_d = (_c = atk.pubVolatiles) === null || _c === void 0 ? void 0 : _c.protectChain) !== null && _d !== void 0 ? _d : 0;
        ...
        case "perish-song": {
            ...
            const atkAbility = (((_e = atk.team[0]) === null || _e === void 0 ? void 0 : _e.ability) || "").toLowerCase();
            ...
            const defAbility = (((_f = def.team[0]) === null || _f === void 0 ? void 0 : _f.ability) || "").toLowerCase();
...
function endOfTurn(ctx) {
    ...
    for (const s of [ctx.a, ctx.b]) {
        ...
        if ((_a = s.pubVolatiles) === null || _a === void 0 ? void 0 : _a.recharge) {
            s.pubVolatiles.recharge = false;
            s.publicVolatilesChanged = true;
        }
        // Clear per-turn shield; chain persists (affects next use)
        if ((_b = s.pubVolatiles) === null || _b === void 0 ? void 0 : _b.protectActive) {
            s.pubVolatiles.protectActive = false;
            s.publicVolatilesChanged = true;
        }
        else if (((_d = (_c = s.pubVolatiles) === null || _c === void 0 ? void 0 : _c.protectChain) !== null && _d !== void 0 ? _d : 0) > 0) {
    ...
    for (const uid of [ctx.a.uid, ctx.b.uid]) {
        ...
        if ((field.safeguard[uid] || 0) > 0)
            field.safeguard[uid]--;
        if ((field.mist[uid] || 0) > 0)
            field.mist[uid]--;
    }
    // Perish Song countdown
    for (const s of [ctx.a, ctx.b]) {
        if (((_e = s.pubVolatiles) === null || _e === void 0 ? void 0 : _e.perishTurns) != null) {
...
function label(user, ctx) {
    const s = user === "A" ? ctx.a : ctx.b;
    return s.active.species || (user === "A" ? "P1" : "P2");
}
function sideLabel(s, ctx) {
...
exports.onReplacementCreate = functions.database
...
exports.createBattleWithTeams = functions.https.onCall(async (data, context) => {
    ...
    const convertTeamToPokemon = async (team) => {
        var _a, _b;
        const convertedTeam = [];
        for (const slot of team) {
            ...
            try {
                ...
                const calcStat = (base) => Math.floor(((2 * base + 31 + 63) * level / 100) + 5);
                const pokemon = {
                    ...
                    item: slot.item || '',
                    ability: slot.ability || (((_b = (_a = pokemonData.abilities.find((a) => a.is_default)) === null || _a === void 0 ? void 0 : _a.ability) === null || _b === void 0 ? void 0 : _b.name) || ''),
...
async function tryResolveTurn(bid, turn) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j;
    ...
    const reserve = await opRef.transaction(v => v !== null && v !== void 0 ? v : opId);
    if (!reserve.committed || reserve.snapshot.val() !== opId)
        return;
    // 3) Flip to resolving
    const flip = await metaRef.transaction((m) => {
        if (!m || m.phase !== "choosing")
            return;
        m.phase = "resolving";
        return m;
    });
    if (!flip.committed || ((_a = flip.snapshot.val()) === null || _a === void 0 ? void 0 : _a.phase) !== "resolving")
        return;
    ...
    const privA = privASnap.val();
    const privB = privBSnap.val();
    // 5) Version guard
    const cvA = (_b = choices[p1]) === null || _b === void 0 ? void 0 : _b.clientVersion;
    const cvB = (_c = choices[p2]) === null || _c === void 0 ? void 0 : _c.clientVersion;
    ...
    const ctx = {
        ...
        rng: { seed: (_e = (_d = meta.rng) === null || _d === void 0 ? void 0 : _d.seed) !== null && _e !== void 0 ? _e : 1, cursor: (_g = (_f = meta.rng) === null || _f === void 0 ? void 0 : _f.cursor) !== null && _g !== void 0 ? _g : 0 },
        logs: []
    };
    ...
    for (const act of queue) {
        if (ctx.a.active.hp <= 0 || ctx.b.active.hp <= 0)
            break; // active fainted; stop executing more actions
    ...
    const needsReplace = {};
    if (ctx.a.active.hp <= 0)
        needsReplace[p1] = true;
    if (ctx.b.active.hp <= 0)
        needsReplace[p2] = true;
    ...
    update[`/battles/${bid}/public/${p1}/active/status`] = (_h = ctx.a.active.status) !== null && _h !== void 0 ? _h : null;
    if (ctx.a.publicVolatilesChanged)
        update[`/battles/${bid}/public/${p1}/active/volatiles`] = ctx.a.pubVolatiles;
    update[`/battles/${bid}/public/${p2}/active/hp`] = { cur: Math.max(0, ctx.b.active.hp), max: ctx.b.active.maxHp };
    update[`/battles/${bid}/public/${p2}/active/status`] = (_j = ctx.b.active.status) !== null && _j !== void 0 ? _j : null;
    if (ctx.b.publicVolatilesChanged)
        update[`/battles/${bid}/public/${p2}/active/volatiles`] = ctx.b.pubVolatiles;
    if (ctx.lastResultSummary)
        update[`/battles/${bid}/public/lastResultSummary`] = ctx.lastResultSummary;
    ...
    if (Object.keys(needsReplace).length)
        update[`/battles/${bid}/meta/needsReplace`] = needsReplace;
    else
        update[`/battles/${bid}/meta/needsReplace`] = null;
    // Meta
...
/** ------------------- Replacement phase resolver ------------------- */
async function tryResolveReplacements(bid, turn) {
    var _a;
    const metaRef = db.ref(`/battles/${bid}/meta`);
    ...
    const waiting = Object.keys(need).filter(uid => !reps[uid]);
    if (waiting.length)
        return;
    ...
    for (const uid of Object.keys(need)) {
        const who = uid === p1 ? "A" : "B";
        const s = who === "A" ? ctx.a : ctx.b;
        const opp = who === "A" ? ctx.b : ctx.a;
        const idx = Math.max(1, Math.min((((_a = reps[uid]) === null || _a === void 0 ? void 0 : _a.switchToIndex) | 0), s.team.length - 1));
    ...
    if (ctx.a.publicVolatilesChanged)
        updates[`/battles/${bid}/public/${p1}/active/volatiles`] = ctx.a.pubVolatiles;
    ...
    if (ctx.b.publicVolatilesChanged)
        updates[`/battles/${bid}/public/${p2}/active/volatiles`] = ctx.b.pubVolatiles;
...
/** ---------- Replay exporter ---------- */
exports.exportReplay = functions.https.onCall(async (data, context) => {
    ...
    const p1 = (_c = (_b = meta.players) === null || _b === void 0 ? void 0 : _b.p1) === null || _c === void 0 ? void 0 : _c.uid, p2 = (_e = (_d = meta.players) === null || _d === void 0 ? void 0 : _d.p2) === null || _e === void 0 ? void 0 : _e.uid;
    if (uid !== p1 && uid !== p2) {
    ...
    const replay = {
        ...
        finalMeta: {
            phase: meta.phase,
            turn: meta.turn,
            winnerUid: (_f = meta.winnerUid) !== null && _f !== void 0 ? _f : null,
            endedReason: (_g = meta.endedReason) !== null && _g !== void 0 ? _g : null
        }
    };
    return replay; // returned as JSON payload to client
});
/** ---------- (Optional) Timeout sweeper using absolute deadline ---------- */
exports.sweepTurnTimeouts = functions.pubsub
