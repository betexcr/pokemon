...
# Process all Pokemon entries, keeping only the first occurrence of each Japanese name
entries = []
seen_names = set()
...
for row in pokemon_data:
    ...
    # Skip duplicates
    if japanese_name in seen_names:
        print(f'Skipping duplicate: {japanese_name} ({english_name})')
        continue
    
    seen_names.add(japanese_name)
    
    # Generate pronunciation (basic approximation)
    pronunciation = f'Basic pronunciation for {english_name}'.replace("'", "\\'")
...
print(f'Generated {len(entries)} unique TypeScript entries')
...
# Add Pokemon ID mappings (only for unique entries)
seen_names_mapping = set()
seen_ids = set()
for row in pokemon_data:
    dex_num = int(row['\ufeffDexNumber'])  # Handle BOM in header
    japanese_name = row['JapaneseName'].split(' (')[0]
    
    # Skip duplicates in mapping too
    if japanese_name in seen_names_mapping or dex_num in seen_ids:
        continue
    
    seen_names_mapping.add(japanese_name)
    seen_ids.add(dex_num)
...
# Write the new file
with open('src/lib/japaneseNames_clean.ts', 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f'Created japaneseNames_clean.ts with {len(entries)} unique Pokemon entries')