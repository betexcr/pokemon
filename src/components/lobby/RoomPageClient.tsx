...
export default function RoomPageClient({ roomId }: RoomPageClientProps) {
            ...
            .map((slot) => {
                ...
                const moves = rawMoves
                    .map((move: any) => {
                        ...
                        // If it's already a full move object (has power, type, etc), keep it
                        if (move.power !== undefined || move.type || move.category) {
                            return move;
                        }
    ...
    const toFirestoreTeam = useCallback((slots: StoredSlot[]): Array<Record<string, unknown>> => {
        return slots.map((slot) => {
            // Preserve full move objects if available, otherwise fallback to names
            const moves = slot.moves
                .map((move) => {
                    if (typeof move === 'string') return move;
                    if (move && typeof move === 'object') {
                        // If it has detailed stats (power, type, etc), keep them
                        if ('power' in move || 'type' in move || 'category' in move) {
                            return move;
                        }
                        // Otherwise just return the name/id wrapper
                    ...
                    <div className="flex flex-wrap gap-3">
                        {(isHost || isGuest) && (
                            <button
                                ...
                                disabled={busy}
                                className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${(isHost ? room.hostReady : room.guestReady)
                    ...
                    {isHost && (
                        <div className="rounded-lg border border-dashed border-border p-4">
                            ...
                            <button
                                ...
                                className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${bothReady ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-muted text-muted-foreground cursor-not-allowed'
                                    } ${busy ? 'opacity-60' : ''}`}
                            >
