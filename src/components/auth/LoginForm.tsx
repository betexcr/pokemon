...
export default function LoginForm({ onToggleMode, onSuccess }: LoginFormProps) {
  ...
  return (
    <div className="w-full">
      {/* Title */}
      <h1 className="text-center text-2xl sm:text-3xl lg:text-[32px] leading-none font-extrabold text-text mb-4 sm:mb-6">
        Sign In
      </h1>
      ...
      <form onSubmit={handleSubmit} className="mx-auto w-full">
        ...
        <label className="block mb-3">
          ...
          <input
            ...
            className="w-full h-12 sm:h-14 md:h-16 rounded-xl sm:rounded-2xl border border-border px-4 sm:px-5 text-sm sm:text-base font-medium outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 hover:border-border"
        ...
        <label className="block mb-3">
          ...
          <div className="relative">
            ...
            <button
              ...
              className="absolute inset-y-0 right-4 sm:right-5 text-sm text-muted hover:text-text transition-colors duration-200 px-2"
        ...
        >
          {loading ? (
            <div className="flex items-center justify-center gap-2">
              <img src="/loading.gif" alt="Loading" className="h-4 w-4 sm:h-5 sm:w-5" />
      ...
      <button
        ...
        disabled={loading || googleLoading || microsoftLoading || twitterLoading}
        className="w-full h-12 sm:h-14 md:h-16 rounded-xl sm:rounded-2xl border border-border bg-surface hover:bg-surface/80 flex items-center justify-center gap-3 sm:gap-4 shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 mt-4 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
      >
        {googleLoading ? (
          <div className="flex items-center gap-2">
            <img src="/loading.gif" alt="Loading" className="h-4 w-4 sm:h-5 sm:w-5" />
            <span className="text-text font-semibold text-sm sm:text-base">Signing in...</span>
          </div>
        ) : (
          <>
            <div 
