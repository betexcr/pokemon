...
test.describe('Authenticated Users Pokemon Battle', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    console.log('🎮 Setting up authenticated users Pokemon battle test');
  ...
  test.afterAll(async () => {
    console.log('🧹 Cleaning up authenticated users Pokemon battle test');
  ...
  async function loginUser(page: Page, email: string, password: string, userType: string) {
    ...
    for (const selector of loginSelectors) {
      try {
        ...
        if (await button.isVisible()) {
          console.log(`✅ ${userType} found login button: ${selector}`);
          await button.click();
          console.log(`🔐 ${userType} clicked login button`);
          loginClicked = true;
          break;
        }
      } catch (error) {
    ...
    if (!loginClicked) {
      console.log(`ℹ️ ${userType} no login button found, looking for auth form directly`);
    }
  ...
  test('Complete Authenticated Users Pokemon Battle Flow', async () => {
    console.log('🎮 Starting complete authenticated users Pokemon battle flow');
    ...
    if (!roomCreated) {
      ...
      const roomId = `auth-users-room-${Date.now()}`;
    ...
    await hostPage.screenshot({ path: 'test-results/host-auth-users-room.png', fullPage: true });
    await guestPage.screenshot({ path: 'test-results/guest-auth-users-room.png', fullPage: true });
    
    console.log('📸 Screenshots taken: host-auth-users-room.png, guest-auth-users-room.png');
    ...
    await hostPage.screenshot({ path: 'test-results/host-auth-users-battle.png', fullPage: true });
    await guestPage.screenshot({ path: 'test-results/guest-auth-users-battle.png', fullPage: true });
    
    console.log('📸 Battle screenshots taken: host-auth-users-battle.png, guest-auth-users-battle.png');
    ...
    console.log('🎉 Authenticated users Pokemon battle flow test completed!');
