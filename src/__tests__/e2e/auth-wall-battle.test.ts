...
test.describe('Auth Wall Pokemon Battle', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    console.log('🎮 Setting up auth wall Pokemon battle test');
  ...
  test.afterAll(async () => {
    console.log('🧹 Cleaning up auth wall Pokemon battle test');
    await hostContext?.close();
    await guestContext?.close();
  });

  async function handleAuthWall(page: Page, email: string, userType: string) {
    console.log(`\n🔐 Handling auth wall for ${userType}: ${email}`);
    ...
    // Wait for auth to complete (even if it fails, we'll continue)
    await page.waitForTimeout(3000);
    
    console.log(`✅ ${userType} auth wall handling completed`);
    return emailFilled && passwordFilled && submitted;
  }

  test('Auth Wall Pokemon Battle Flow', async () => {
    console.log('🎮 Starting auth wall Pokemon battle flow');
    ...
    // Step 2: Navigate to lobby again (in case auth succeeded)
    ...
    if (!roomCreated) {
      ...
      const roomId = `auth-wall-room-${Date.now()}`;
    ...
    await hostPage.screenshot({ path: 'test-results/host-auth-wall-room.png', fullPage: true });
    await guestPage.screenshot({ path: 'test-results/guest-auth-wall-room.png', fullPage: true });
    
    console.log('📸 Screenshots taken: host-auth-wall-room.png, guest-auth-wall-room.png');
    ...
    await hostPage.screenshot({ path: 'test-results/host-auth-wall-battle.png', fullPage: true });
    await guestPage.screenshot({ path: 'test-results/guest-auth-wall-battle.png', fullPage: true });
    
    console.log('📸 Battle screenshots taken: host-auth-wall-battle.png, guest-auth-wall-battle.png');
    ...
    console.log('🎉 Auth wall Pokemon battle flow test completed!');
