...
test.describe('Bypass Auth Pokemon Battle', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    console.log('🎮 Setting up bypass auth Pokemon battle test');
  ...
  test.afterAll(async () => {
    console.log('🧹 Cleaning up bypass auth Pokemon battle test');
    await hostContext?.close();
    await guestContext?.close();
  });

  async function bypassAuth(page: Page, email: string, userType: string) {
    console.log(`\n🔐 Bypassing auth for ${userType}: ${email}`);
    ...
    // If we hit the auth wall, we'll bypass it by mocking the auth state
    const currentUrl = page.url();
    console.log(`📍 ${userType} current URL: ${currentUrl}`);
    
    // Check if we're on an auth wall page
    const authWallText = await page.locator('body').textContent();
    if (authWallText?.includes('Authentication Required')) {
      console.log(`🚧 ${userType} hit auth wall, bypassing...`);
      
      // Mock authentication by setting up localStorage and sessionStorage
      await page.evaluate((userEmail) => {
        ...
        // Set up mock Firebase auth state
        window.firebaseAuthUser = mockUser;
        
        // Mock the auth state change
        if (window.firebaseAuthStateChanged) {
          window.firebaseAuthStateChanged(mockUser);
        }
        
        console.log(`🔐 ${userEmail} mock auth bypassed`);
      }, email);
      
      // Reload the page to apply the mock auth
      await page.reload();
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(2000);
      
      console.log(`✅ ${userType} auth bypassed and page reloaded`);
    } else {
      console.log(`✅ ${userType} no auth wall detected, proceeding`);
    }
    
    return true;
  }

  test('Bypass Auth Pokemon Battle Flow', async () => {
    console.log('🎮 Starting bypass auth Pokemon battle flow');
    ...
    if (!roomCreated) {
      ...
      const roomId = `bypass-auth-room-${Date.now()}`;
    ...
    await hostPage.screenshot({ path: 'test-results/host-bypass-auth-room.png', fullPage: true });
    await guestPage.screenshot({ path: 'test-results/guest-bypass-auth-room.png', fullPage: true });
    
    console.log('📸 Screenshots taken: host-bypass-auth-room.png, guest-bypass-auth-room.png');
    ...
    await hostPage.screenshot({ path: 'test-results/host-bypass-auth-battle.png', fullPage: true });
    await guestPage.screenshot({ path: 'test-results/guest-bypass-auth-battle.png', fullPage: true });
    
    console.log('📸 Battle screenshots taken: host-bypass-auth-battle.png, guest-bypass-auth-battle.png');
    ...
    console.log('🎉 Bypass auth Pokemon battle flow test completed!');
