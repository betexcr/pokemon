// Battle Stadium Singles (BSS) Official usage data adapter
// Parses data from HOME, Pikalytics, and other BSS sources
...
export class BSSOfficialAdapter extends AbstractAdapter {
  
  canHandle(config: IngestionConfig): boolean {
    return config.platform === 'BSS_OFFICIAL';
  }
  
  getSupportedFormats(): Format[] {
    return [
      'BSS_SERIES_8', 'BSS_SERIES_9', 'BSS_SERIES_12', 'BSS_SERIES_13',
      'BSS_REG_C', 'BSS_REG_D', 'BSS_REG_E', 'BSS_REG_I'
    ];
  }
  ...
  async parse(config: IngestionConfig, content: string): Promise<AdapterResult> {
    ...
    try {
      // BSS data is typically in JSON format from HOME API or CSV from Pikalytics
      if (this.isJSONFormat(content)) {
        ...
        errors.push('Unsupported file format. Expected JSON or CSV.');
        return {
          rows: [],
          errors,
          warnings,
  ...
  private async parseJSON(content: string): Promise<AdapterResult> {
    ...
    try {
      ...
      // Handle different JSON structures for BSS data
      let usageData: any[] = [];
      
      if (Array.isArray(data)) {
        usageData = data;
      } else if (data.rankings && Array.isArray(data.rankings)) {
        usageData = data.rankings;
      } else if (data.usage && Array.isArray(data.usage)) {
        usageData = data.usage;
      } else if (data.data && Array.isArray(data.data)) {
        usageData = data.data;
      } else if (data.bss && Array.isArray(data.bss)) {
        usageData = data.bss;
      } else if (data.battle_stadium_singles && Array.isArray(data.battle_stadium_singles)) {
        usageData = data.battle_stadium_singles;
      } else {
        errors.push('Unknown JSON structure. Expected array or object with rankings/usage/data/bss array.');
  ...
  private parseJSONEntry(entry: any, index: number): UsageRow | null {
    try {
      // Handle different field names for BSS data
      const name = entry.name || entry.pokemon || entry.pokemon_name || entry.pokemonName || entry.species;
      const usage = entry.usage || entry.usage_percent || entry.usagePercent || entry.percent || entry.usage_rate;
      const rank = entry.rank || entry.position || entry.pos || entry.ranking;
      const sample = entry.sample || entry.battles || entry.raw || entry.raw_usage || entry.total_battles;
      ...
      // Parse substats if available (BSS often includes moves and items)
  ...
  private parseSubstats(entry: any): any {
    ...
    // Parse teammates (less common in BSS but sometimes available)
    ...
    // Parse spreads (nature + EVs)
    if (entry.spreads && Array.isArray(entry.spreads)) {
      substats.spreads = entry.spreads.map((spread: any) => ({
        nature: spread.nature || spread.nature_name,
        evs: spread.evs || spread.ev_spread || spread.stat_spread,
        pct: typeof spread.usage === 'number' ? spread.usage : this.parsePercentage(spread.usage?.toString() || '0')
      }));
    }
  ...
  private async parseCSV(content: string): Promise<AdapterResult> {
    ...
    // Parse header with BSS-specific column names
    const header = lines[0].split(',').map(col => col.trim().toLowerCase());
    const rankIndex = this.findColumnIndex(header, ['rank', '#', 'position', 'ranking']);
    const nameIndex = this.findColumnIndex(header, ['pokemon', 'name', 'pokémon', 'pokemon_name', 'species']);
    const usageIndex = this.findColumnIndex(header, ['usage', '%', 'usage%', 'usage_percent', 'usage_rate']);
    const sampleIndex = this.findColumnIndex(header, ['raw', 'raw usage', 'sample', 'battles', 'usage_raw', 'total_battles']);
  ...
  private getPokemonIdFromName(name: string): number | null {
    // Use the same mapping as other adapters
